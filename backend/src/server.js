...
function log(level, msg, fields = {}) {
  ...
  console[level === 'error' ? 'error' : 'log'](
    `[${ts}] [${level.toUpperCase()}] ${msg}${extras ? ' ' + extras : ''}`
  );
}
...
app.use(
  cors({
    origin: (origin, cb) =>
      !origin || allowedOrigins.has(origin) ? cb(null, true) : cb(new Error('CORS')),
...
app.post('/api/setup', setupLimiter, (req, res) => {
  if (!needsSetup()) return res.status(409).json({ error: 'Setup already completed' });
  ...
  if (!token) return res.status(401).json({ error: 'Invalid setup token' });
...
app.use(
  '/api/import',
  rateLimit({ windowMs: 10 * 60_000, max: 5, standardHeaders: true, legacyHeaders: false })
);
...
app.post('/api/admin/users', requireAdmin, (req, res) => {
  ...
  res
    .status(201)
    .json({ id: row.id, username: row.username, role: row.role, created_at: row.created_at });
});
