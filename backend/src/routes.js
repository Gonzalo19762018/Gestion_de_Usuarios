...
import {
  validateMonthBody,
  validateImportMonth, validateImportDeferral, validateImportAccount,
  IMPORT_MAX,
} from './validators.js';
...
router.get('/events', (req, res) => {
  ...
  req.on('close', () => { clearInterval(heartbeat); removeClient(res, req.user); });
});
...
router.put('/months/:key', (req, res) => {
  const body = req.body;
  if (!body || body.key !== req.params.key)
...
router.put('/deferrals/:id', (req, res) => {
  ...
  if (body.cuotas !== undefined && (!Number.isInteger(body.cuotas) || body.cuotas < 1 || body.cuotas > 120))
...
router.post('/import', (req, res) => {
  ...
  if ((ms?.length > IMPORT_MAX) || (ds?.length > IMPORT_MAX) || (as?.length > IMPORT_MAX))
    return res.status(400).json({ error: `Import arrays must not exceed ${IMPORT_MAX} items each` });
  ...
  const run = getDb().transaction(() => {
    ...
    if (Array.isArray(ms)) ms.forEach(m => { months.upsert(req.user, m);    imported.months++;    });
    if (Array.isArray(ds)) ds.forEach(d => { deferrals.upsert(req.user, d); imported.deferrals++; });
    if (Array.isArray(as)) as.forEach(a => { accounts.upsert(req.user, a);  imported.accounts++;  });
    return imported;
  });
  const imported = run();
