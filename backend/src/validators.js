...
export function validateMonthBody(body) {
  ...
  if (body.transactions !== undefined) {
    if (!Array.isArray(body.transactions)) return 'transactions must be an array';
    if (body.transactions.length > 500) return 'too many transactions (max 500)';
    for (const t of body.transactions) {
      if (typeof t.id !== 'number') return 'transaction id must be a number';
      if (typeof t.name !== 'string' || t.name.length > 200)
        return 'transaction name must be a string ≤200 chars';
      ...
      if (t.type !== undefined && typeof t.type !== 'string')
        return 'transaction type must be a string';
      if (t.cat !== undefined && typeof t.cat !== 'string')
        return 'transaction cat must be a string';
      if (t.date !== undefined && typeof t.date !== 'string')
        return 'transaction date must be a string';
    }
  }
  if (body.budgets !== undefined) {
    ...
    for (const [k, v] of Object.entries(body.budgets)) {
      ...
      if (
        v.limit !== undefined &&
        (typeof v.limit !== 'number' || !Number.isFinite(v.limit) || v.limit < 0)
      )
  ...
  if (body.income !== undefined) {
    ...
    if (
      body.income.amount !== undefined &&
      (typeof body.income.amount !== 'number' || body.income.amount < 0)
    )
...
export function validateImportMonth(m, idx) {
  if (!m || typeof m !== 'object' || Array.isArray(m)) return `months[${idx}]: must be an object`;
...
export function validateImportDeferral(d, idx) {
  ...
  if (!isPositiveFinite(d.id)) return `deferrals[${idx}]: id must be a positive number`;
  ...
  if (!d.date || !DATE_RE.test(d.date)) return `deferrals[${idx}]: date must be YYYY-MM-DD`;
...
export function validateImportAccount(a, idx) {
  if (!a || typeof a !== 'object' || Array.isArray(a)) return `accounts[${idx}]: must be an object`;
  if (!isPositiveFinite(a.id)) return `accounts[${idx}]: id must be a positive number`;
